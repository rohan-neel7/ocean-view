...
- **Phase 7.9 (Complete)**: Cesium Rendering Stability & WebGL Lifecycle (Single-owner viewer lifecycle, Named Active Governor Holds, Google 3D Tiles Fallback Guard).
- **Phase 8.1 (Complete)**: Current Velocity Visualization Recovery (Real ANDRO U/V Ingestion, Canonical Typed Vectors, `depthFailMaterial` Polyline Arrows, Transparent Particle Compositing).
- **Phase 8.2 (Complete)**: Scientific Visualization Readability Engine (`scientificRenderPolicy.js`, Adaptive Vector Decimation, Bounded Visual Scaling, Bilinear Scalar Canvas Interpolation, Multi-Layer Opacity Hierarchy, Platform Color Identities, and Inspection Probes).
- **Phase 8.2A (Complete)**: Scientific Coordinate & Render-Offset Integrity (`depthCoordinates.js`, deterministic physical depth mapping vs display offsets $+5\text{m}\dots+12\text{m}$, negative ellipsoidal subsurface mapping, and strict scientific depth preservation).
- **Phase 8.3 (Complete)**: Spatial Scientific Investigation Workspace (`analysisLocation.js`, Cartographic ocean-only click picking, land rejection, bounded context windows, multi-format coordinate search + basin autocomplete, direct bilinear model sampling, geodesic in-situ observation discovery, 1-click model-observation comparison, and Cesium target marker).
